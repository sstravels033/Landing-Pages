...
https.get('https://raw.githubusercontent.com/datameet/maps/master/Country/india-composite.geojson', (resp) => {
    ...
    resp.on('end', async () => {
        try {
            ...
            const svgStyles = `
                <style>
                    .india-outline { fill: none; stroke: #e2e8f0; stroke-width: 3; opacity: 0.7; }
                    .city-dot { fill: #ff2d55; opacity: 1; }
                    .city-hub { fill: #000000; }
                    .city-label { fill: #1e293b; font-size: 16px; font-family: sans-serif; text-anchor: middle; font-weight: 800; stroke: white; stroke-width: 4px; paint-order: stroke; }
                    .hub-label { font-size: 18px; fill: #000000; stroke: white; stroke-width: 4px; paint-order: stroke; }
                    .route-line { stroke: #cbd5e1; stroke-width: 1; stroke-dasharray: 2 4; opacity: 0.3; }
                </style>
            `;
            ...
            svgHtml += '<circle class="city-dot city-hub" cx="' + hyd[0] + '" cy="' + hyd[1] + '" r="12" />';
            svgHtml += '<text x="' + hyd[0] + '" y="' + (hyd[1] + 25) + '" class="city-label hub-label">HYDERABAD</text>';
            
            for (const [city, coords] of Object.entries(cities)) {
                if (city === 'Hyderabad') continue;
                const pt = projection(coords);
                svgHtml += '<circle class="city-dot" cx="' + pt[0] + '" cy="' + pt[1] + '" r="8" />';
                svgHtml += '<text x="' + pt[0] + '" y="' + (pt[1] + 18) + '" class="city-label">' + city + '</text>';
            }
            ...
            const imagePath = 'samples/sample3.webp';
